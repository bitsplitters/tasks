---
title: Community & support
description: The official Discord server for TTM — get help, report bugs, request features and shape what comes next.
group: Help & community
order: 9.7
---

Team Tasks Manager has an **official Discord community server**, published by bitsplitters. It's the fastest way to get help, tell us what's missing and stay ahead of what's coming — no need to add the bot to join.

<a class="btn" href="https://discord.gg/ec7j5YRc4R">Join the Discord server &rarr;</a>

## What you can do there

- **Get help & support** — stuck on a setup, a permission or a webhook? Ask in the support channels and get an answer from us or another admin who's already been there.
- **Report a bug** — found something off? Open a bug report so we can reproduce and fix it. Clear steps and a screenshot go a long way.
- **Share feedback & request features** — tell us what would make TTM better for your team. A lot of the roadmap comes straight from these conversations.
- **Follow announcements & roadmap** — see new releases and what's planned, all in one place.
- **Join the beta** — want early access? Try features before they ship (also from the `/beta` command inside the bot).
- **Meet other teams** — swap tips with other server admins running shared checklists the same way you do.

<div class="callout"><div class="callout-t">Good to know</div>The server is dedicated to TTM but published by <strong>bitsplitters</strong>. Everyone's welcome to drop in, ask questions and follow along — even before you add the bot to your own server.</div>

## Prefer email?

For anything private — data requests, account matters or partnerships — write to <a href="mailto:info@bitsplitters.app">info@bitsplitters.app</a>. For public how-to questions and bug reports, the Discord server is quicker.
